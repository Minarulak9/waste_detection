...
Although the current system achieves strong results for binary waste classification, several natural extensions would enhance its capability, coverage, and impact on the real-world. This chapter outlines the most promising directions for future development.

\section{Multi-Category Waste Classification}

The current system classifies waste into two categories (bio/non-bio). In real-world waste management systems ,it require finer-grained segregation like:
\begin{itemize}
    \item \textbf{Paper and Cardboard} (recyclable, separate processing from plastic)
    \item \textbf{Glass} (requiring a dedicated recycling stream)
    \item \textbf{Metal} (high-value recyclable)
    \item \textbf{Plastic} (multiple sub-types like PET, HDPE, PVC)
...
\begin{itemize}
    \item \textbf{Android/ios app:} Camera capture $\rightarrow$ on-device inference $\rightarrow$ classification result will be displayed.
    \item \textbf{Model Optimisation:} Export to ONNX or tensorFlow Lite for mobile runtime feature , apply quantisation (INT8) to reduce model size below 5\,MB.
    \item \textbf{Offline capability:} on remote area ,the On-device inference ensures operation without internet connectivity.
\end{itemize}
...
Smart routing systems fed by accurate bin-state data can deliver significant reductions in collection route distances and fuel consumption, representing substantial operational savings for waste management agencies.
...
The current 3,000 images dataset, while it sufficient for binary classification, could be expanded:
\begin{itemize}
    \item Increase to 10,000+ raw images for improved generalisation accuracy.
...
\begin{itemize}
    ...
    \item \textbf{Uncertainty estimation:} It is  Flagging low-confidence predictions for human review rather than automated action.
\end{itemize}
