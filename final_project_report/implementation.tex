...
\begin{itemize}
    ...
    \item Python:3.10 or a later version
...
This file is the single point of configuration that connects the dataset to the Ultralytics training pipeline. The two class names are \texttt{Bio} (class ID 0)and \texttt{Non Bio}(class ID 1), matches to the biodegradable and non-biodegradable categories.
...
The eight-step ablation study was implemented as eight separate training scripts, each corresponding to one experimental configuration.All scripts share the same datasets, image sizes, batch sizes and optimiser settings to ensure fair comparison. The differences between scripts are:

\begin{itemize}
    \item \textbf{Steps 1--2 (Baseline) :} Here Standard YOLO11n and YOLO11s trained 50 epochs without augmentation and without any custom loss or neck modification.
    \item \textbf{Step 3 (Augmentation) :} Here YOLO11s trained 50 epochs on the augmented 9,576-image dataset, with no architectural modifications.
    ...
    \item \textbf{Step 6 (Deformable Conv) :} Here Deformable convolutional layers added to the Step 5 configuration. 50 epochs.
    ...
    \item \textbf{Step 8 (Final, 100 epochs):} Here Identical Architecture to Step 7, trained for 100 epochs.This is the final production model.
\end{itemize}
...
\textbf{Confidence and IoU thresholds:} Two sidebar sliders allow the users to adjust the detection confidence threshold (default : 0.25) and NMS IoU threshold (default : 0.45) without restarting the application.
