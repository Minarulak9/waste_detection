...
ConvoWaste \cite{convowaste2023} presented an end-to-end automated waste segregation system combining a deep CNN classifier with servo-motor-actuated bin control and ultrasonic fill-level sensors. The system was designed for multi-category classification (up to six waste types) with physical bin actuation triggered by the classification output. The CNN achieved approximately 89\% classification accuracy on a controlled test set. While representing a significant step toward complete automation, limitations include: dependency on a fixed-angle overhead camera, inability to detect multiple waste items simultaneously (single-item classification, not detection), and hardware dependency making the system costly to deploy at scale. This project draws inspiration from the end-to-end automation concept while focusing on improving the core classification accuracy through modern object detection.
...
\subsection{A Systematic Review of AI-Based Techniques for Solid Waste Management (2025)}
...
\subsection{Waste Management 2.0: IoT with LoRaWAN (2024)}

This work \cite{iot_ai2024} proposed a large-scale smart waste management infrastructure integrating IoT sensors, LoRaWAN long-range communication, fill-level monitoring, and AI-based route optimisation. The system demonstrated significant operational improvements: 32\% reduction in collection route distances and 29\% reduction in fuel consumption across a pilot urban deployment. While this work does not focus on waste classification accuracy (it assumes pre-sorted waste bins), it demonstrates the downstream value that accurate classification systems like ours can provide. A classification system achieving 94\% accuracy feeding into such route optimisation infrastructure would significantly improve overall system effectiveness.

\subsection{IoT-Enabled Biodegradable Waste Monitoring (2025)}

This paper \cite{systematic_review2025} presented a sensor-fusion approach using ultrasonic sensors, load cells, and moisture and gas sensors mounted on a NodeMCU microcontroller for biodegradable waste monitoring. The system tracks decomposition progress and bin fill levels in real time, triggering collection and processing alerts. While the sensor-based approach provides complementary information to camera-based classification, it cannot identify waste type at collection time. This work highlights the broader smart waste ecosystem that camera-based classification systems like ours can integrate with.
...
ConvoWaste~\cite{convowaste2023}               & 2023 & Custom               & Classification & Multi  & $\sim$89\% accuracy   & Single-item only; no bounding box prediction \\
IoT + VGG/ResNet~\cite{iot_ai2024}            & 2025 & TrashNet             & Classification & 6      & 91.4\% accuracy       & Clean backgrounds; no spatial localisation \\
...
Waste Management 2.0~\cite{iot_ai2024}         & 2024 & Urban pilot          & IoT / Routing  & ---    & 32\% route reduction  & No image classification component \\
Sensor Fusion (JETIR)                          & 2025 & IoT sensors          & Monitoring     & ---    & Fill-level alerts     & Cannot identify waste type visually \\
