...
The dataset was built from scratch by the team. A total of \textbf{3,000 raw images} were collected through two methods:

\begin{enumerate}
    \item \textbf{Direct Photography (approximately 40\%):} Team members photographed real waste items in diverse settings — kitchen waste bins, outdoor rubbish points, laboratory disposal areas, and institutional canteen waste zones.Images were captured using mobile cameras under different lighting conditions (natural daylight, indoor fluorescent,low-light) and angles(overhead,side-on, close-up).

    \item \textbf{Internet Collection (approximately 60\%):}Additional images were collected from open image repositories to expand diversity of waste item types, backgrounds, and presentation styles. Images were selected to represent real-world scenarios rather than studio or isolated item photographs.
\end{enumerate}
...
All 3,000 images were manually annotated using the \textbf{Roboflow Annotation Platform}.Each annotation consists of an axis-aligned bounding box drawn closely around the each visible waste item,with the class label assigned as either \textit{Bio} or \textit{Non-Bio}.
...
\begin{equation}
\begin{aligned}
c_x &= \frac{\min(x) + \max(x)}{2}, \qquad
c_y = \frac{\min(y) + \max(y)}{2}, \\[6pt]
w   &= \max(x) - \min(x), \qquad\quad\;
h   = \max(y) - \min(y)
\end{aligned}
\end{equation}
...
Mosaic~\cite{bochkovskiy2020yolov4} & Enabled             & Combines four images for multi-scale training \\
...
The standard YOLO neck was replaced with a BiFPN~\cite{tan2020efficientdet} configuration. The rationale for BiFPN stems from the multi-scale nature of waste objects:
\begin{itemize}
    ...
    \item Standard FPN~\cite{lin2017fpn} fuses top-down only, losing fine-grained bottom-up context.
...
Rather than training from random initialisation, the model was initialised from COCO~\cite{lin2014coco} pretrained YOLO11s weights (\texttt{yolo11s.pt}). Transfer learning provides:
\begin{itemize}
...
\textbf{AdamW}~\cite{loshchilov2019adamw} (Adaptive Moment Estimation with Weight Decay) was selected as the optimiser. AdamW decouples weight decay from the gradient update, providing better regularisation than standard Adam.

The learning rate schedule uses \textbf{cosine annealing}:It starts at $lr_0 = 0.01$,it slowly goes down to $lr_f = 0.001$ over 100 epochs. This avoids abrupt learning rate drops and allows fine-grained weight adjustment in later epochs. A 3-epoch warmup phase linearly ramps the learning rate from near-zero to $lr_0$, stabilising early training.
...
Eight configurations were trained across two phases. Steps 1--6 add components incrementally at 50 epochs; Steps 7--8 isolate the architecture-versus-training-time effect:
...
4 & + CBAM~\cite{woo2018cbam} + WIoU & Effect of attention + loss function \\
5 & + BiFPN & \textbf{Effect of bidirectional feature fusion} \\
6 & + Deformable Convolutions~\cite{dai2017deformable} & Effect of spatial adaptivity \\
\midrule
7 & YOLO11s + BiFPN + WIoU (no CBAM), 50 ep & Architecture effect isolated \\
\textbf{8 (Final)} & \textbf{YOLO11s + BiFPN + WIoU, 100 ep} & \textbf{Best config at full training} \\
\bottomrule
\end{tabular}}
\end{table}

Steps 1--7 all used 50 epochs for direct comparison. The final model (Step 8) used 100 epochs to validate the benefit of extended training for the best-performing architecture.
...
R = \frac{TP}{TP + FN}
\end{equation}
Fraction of ground-truth instances detected. High recall means some missed detections.

\subsection{Average Precision (AP)}
This is the area under the curve that shows how precise the Precision-Recall is for one class. It is calculated by looking at all the confidence thresholds, which go from 0 to 1.
...
Mean Average Precision is calculated over IoU levels from 0.50, to 0.95 with a step of 0.05. This gives a test of how well the localisation is done.

The IoU thresholds are used to check the localisation quality.

Mean AP is averaged over these thresholds.
